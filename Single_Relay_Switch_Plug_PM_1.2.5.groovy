...
def initialize(){
    ...
    if (settings.relayType.toInteger() == 1 ) {
        ...
        if ( device.currentValue("power") == null ) sendEvent(name: "power", value: 0, descriptionText: "Power (watts) events have been enabled", unit: "Watts" )
        device.deleteCurrentState("current")
        device.deleteCurrentState("voltage")
        device.deleteCurrentState("apparentPower")
		device.deleteCurrentState("energy")
    ...
    if (settings.relayType.toInteger() == 2 ) {
        ...
        if ( device.currentValue("current") == null ) sendEvent(name: "current", value: 0, descriptionText: "Current events have been enabled", unit: "Amps")
        if ( device.currentValue("voltage") == null ) sendEvent(name: "voltage", value: 0, descriptionText: "Voltage events have been enabled", unit: "Volts")
        if ( device.currentValue("power") == null ) sendEvent(name: "power", value: 0, descriptionText: "Power (watts) events have been enabled", unit: "Watts")
        device.deleteCurrentState("apparentPower")
		device.deleteCurrentState("energy")
        device.deleteCurrentState("energyToday")
    ...
    if (settings.relayType.toInteger() == 3 ) {
        ...
        if ( device.currentValue("voltage") == null ) sendEvent(name: "voltage", value: 0, descriptionText: "Voltage events have been enabled", unit: "Volts")
        if ( device.currentValue("power") == null ) sendEvent(name: "power", value: 0, descriptionText: "Power (watts) events have been enabled", unit: "Watts")
        if ( device.currentValue("apparentPower") == null ) sendEvent(name: "apparentPower", value: 0, descriptionText: "Apparent Power events have been enabled", unit: "VA")
        if ( device.currentValue("energyToday") == null ) sendEvent(name: "energyToday", value: 0, descriptionText: "Energy Today events have been enabled", unit: "kWh" )
        if ( device.currentValue("energyToday") == null ) sendEvent(name: "energy", value: 0, unit: "kWh", isStateChange: true )
        if ( device.currentValue("energyTotal") == null ) sendEvent(name: "energyTotal", value: 0, descriptionText: "Energy Total events have been enabled", unit: "kWh")
        if ( device.currentValue("energyYesterday") == null ) sendEvent(name: "energyYesterday", value: 0, descriptionText: "Energy Yesterday events have been enabled", unit: "kWh")
        if ( device.currentValue("powerFactor") == null ) sendEvent(name: "powerFactor", value: 0, descriptionText: "Power Factor events have been enabled")
        if ( device.currentValue("reactivePower") == null ) sendEvent(name: "reactivePower", value: 0, descriptionText: "Reactive events have been enabled", unit: "VAr")
    }
...
def syncTasmota(body){
    ...
    if (elapsed > settings.debounce){
        ...
        if ( switch1?.toInteger() == 0 ) updateChild("1","off", true)
        if ( switch1?.toInteger() == 1 ) updateChild("1","on", true)
        if ( switch2?.toInteger() == 0 ) updateChild("2","off", true)
        if ( switch2?.toInteger() == 1 ) updateChild("2","on", true)
        if ( switch3?.toInteger() == 0 ) updateChild("3","off", true)
        if ( switch3?.toInteger() == 1 ) updateChild("3","on", true)
        if ( switch4?.toInteger() == 0 ) updateChild("4","off", true)
        if ( switch4?.toInteger() == 1 ) updateChild("4","on", true)
        if ( switch5?.toInteger() == 0 ) updateChild("5","off", true)
        if ( switch5?.toInteger() == 1 ) updateChild("5","on", true)
        if ( switch6?.toInteger() == 0 ) updateChild("6","off", true)
        if ( switch6?.toInteger() == 1 ) updateChild("6","on", true)
        if ( switch7?.toInteger() == 0 ) updateChild("7","off", true)
        if ( switch7?.toInteger() == 1 ) updateChild("7","on", true)
        if ( switch8?.toInteger() == 0 ) updateChild("8","off", true)
        if ( switch8?.toInteger() == 1 ) updateChild("8","on", true)
...
def tasmotaInjectRule(){
    ...
    rule3 = rule3 + "ON var15#State\$!%var16% DO backlog ; var16 %var15% ; webquery http://${device.hub.getDataValue("localIP")}:${device.hub.getDataValue("localSrvPortTCP")} POST {'TSync':'True'," + varString2 + "} ENDON "
...
def updateChild(String ep, String status, boolean physical=false)
{
    def typeStr = "digital"
    
    if(physical) typeStr = "physical"
    ...
    if (ep == "1") {
        sendEvent(name: "switch", value: status, descriptionText: "'switch' has been turned ${status}", type: typeStr)
        sendEvent(name: "switch1", value: status, descriptionText: "'switch${ep}' has been turned ${status}", type: typeStr)
        }
    else sendEvent(name: "switch"+ep, value: status, descriptionText: "The switch ${ep} has been turned ${status}", type: typeStr)
    ...
    {
        curdevice?.sendEvent(name: "switch", value: status, type: typeStr)
    }
}
